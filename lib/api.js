...
const resolveApiUrl = (endpoint, { sameOrigin = false } = {}) => {
  if (sameOrigin) return endpoint;
...
export function uploadWithProgress(
  ...
  const url = resolveApiUrl(endpoint, options);
    ...
    xhr.addEventListener("load", () => {
      ...
      try {
        ...
        if (xhr.status === 413) {
          reject(
            new Error(
              "Upload too large (server limit ~1MB). The image was compressed automatically — try a smaller file or contact ops to raise nginx client_max_body_size.",
            ),
          );
          return;
        }
        ...
        reject(
          new Error(
            `Invalid server response (HTTP ${xhr.status})${snippet ? `: ${snippet}` : ""}`,
          ),
        );
        return;
      }
