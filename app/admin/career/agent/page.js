...
import * as agentApi from "../services/careerAgentService";
...
export default function CareerAgentPage() {
  ...
  const canPostJobs = canAdd("career") && !isReadOnly("career");
  const canEditGuidelines = canEdit("career") && !isReadOnly("career");

  useEffect(() => {
    if (authLoading) return;
    if (!canView("career")) router.replace("/admin");
  ...
  useEffect(() => {
    if (authLoading || !canView("career")) return;
    agentApi
      .getAgentStatus()
      .then((s) => {
  ...
  return (
    <>
      <style>{`
        ...
        .ca-inner { max-width: 1280px; margin: 0 auto; }
        .ca-back {
          display: inline-flex;
          color: #4f46e5;
        ...
        .ca-title { margin: 0; font-size: 28px; font-weight: 800; color: #1a1a2e; }
        .ca-subtitle { margin: 6px 0 24px; font-size: 14px; color: #64748b; }
        .ca-card {
          background: #fff;
          border-radius: 12px;
        ...
        .ba-sidebar-head { padding: 12px; border-bottom: 1px solid #e2e8f0; }
        .ba-new-btn {
          width: 100%;
          padding: 10px 12px;
          background: #16a37f;
        ...
        .ba-thread-list { flex: 1; overflow-y: auto; padding: 8px; }
        .ba-thread {
          display: block;
          width: 100%;
          text-align: left;
        ...
        .ba-tab.active { color: #1e40af; font-weight: 700; border-color: #bfdbfe; }
        .ba-tab-close {
          border: none;
          background: transparent;
          color: #94a3b8;
        ...
        .ba-msg-actions { margin-top: 8px; display: flex; gap: 6px; }
        .ba-fb-btn {
          font-size: 12px;
          padding: 4px 8px;
      ...
      <div className="ca-wrap">
        <div className="ca-inner">
          <Link href="/admin/career/jobs" className="ca-back">
            ← Back to jobs
          </Link>
          <h1 className="ca-title">Career Agent</h1>
          <p className="ca-subtitle">
            Chat with AI to create, update, and close job posts. Conversations are
            saved so the agent learns from your feedback.
          </p>

          <div className="ca-card">
            {isReadOnly("career") && <ReadOnlyBanner moduleKey="career" />}
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                {showGuidelines ? "Hide" : "Show"} hiring guidelines
              </button>
            </div>

            {showGuidelines && (
              <div className="ba-guidelines-panel">
                <strong>Hiring brain</strong>
                <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>
                  The agent reads this on every run. Update when job-posting rules change.
                </p>
                <textarea
            ...
            <div className="ba-layout">
              ...
              <div className="ba-main">
                ...
                <div className="ba-fresh">
                  {activeThreadId
                    ? "Viewing a saved chat — open New chat for a fresh session."
                    : "Fresh chat — each page visit starts blank. History stays in the left sidebar."}
                </div>
                <div className="ba-status">
                  <span>
                    {user?.email && <>Signed in as {user.email} · </>}
                    {canPostJobs ? (
                      <span className="ba-badge ok">Can post jobs</span>
                    ) : (
                      <span className="ba-badge draft">View only</span>
                    )}
                    {agentModel ? (
                      <span className="ba-badge draft" style={{ marginLeft: 6 }}>
                        {agentModel}
                      </span>
                    ) : null}
                  </span>
                ...
                <div className="ba-messages">
                  ...
                  {!loadingMessages && messages.length === 0 && (
                    <div className="ba-empty">
                      <div>
                        ...
                        <p style={{ marginTop: 8 }}>
                          &quot;Create an active job for Senior React Developer,
                          3 years exp, Noida, 2 positions&quot;
                        </p>
                        <p style={{ marginTop: 8, fontSize: 13 }}>
                          Or: &quot;List open jobs&quot; / &quot;Close job id 12&quot;
                        </p>
                        <p style={{ marginTop: 8, fontSize: 13 }}>
                          The agent remembers this conversation and learns from
                          your 👍/👎 feedback.
                        </p>
                      </div>
                    </div>
                  ...
                  {sending && <div className="ba-msg assistant">Thinking…</div>}
                ...
                <form className="ba-input-row" onSubmit={handleSend}>
                  <textarea
                    ...
                    placeholder={
                      canPostJobs
                        ? 'e.g. "Post a Digital Marketing Manager role in Noida"'
                        : 'e.g. "List active jobs"'
                    }
                    value={input}
