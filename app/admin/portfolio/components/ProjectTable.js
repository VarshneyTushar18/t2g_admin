...
export default function ProjectTable({ items, onEdit, onDelete }) {
    ...
    useEffect(() => {
        ...
        <table class="display nowrap" style="width:100%">
          <thead>
            <tr>
              <th>ID</th><th>Title</th><th>Image</th>
              <th>Description</th><th>Link</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            ${items.map((item) => `
              <tr>
                ...
                <td>${item.title ?? ""}</td>
                <td>${item.image_url
                    ? `<img src="${item.image_url}" width="80" height="55"
                       style="object-fit:cover;border-radius:6px" />`
                    : "—"}</td>
                <td>${item.description ?? "—"}</td>
                <td>${item.project_link
                    ? `<a href="${item.project_link}" target="_blank"
                       rel="noreferrer" style="color:#4f8ef7;font-size:12px">Link</a>`
                    : "—"}</td>
                <td data-id="${item.id}">
